...
void Context::setMaterial() {
	material.ambient = vec4(1, 0.28, 0.48, 1);
	material.diffuse = vec4(1, 0.28, 0.48, 1);
	material.specular = vec4(1, 0.28, 0.48, 1);
...
void Context::drawParameters() {
	...
	ostringstream s;
#ifdef WIN32
	s << "frametime in ms     : " << msecTime << endl;
	drawString(windowSize.x - 170, windowSize.y - 75, s.str());
	s.str("");
#endif
	string mode = (shadingDemo->getRenderMode() == VERTEX_ARRAY) ?
		"VERTEX_ARRAY" : "VERTEX_BUFFER";
	s << "mode(m) : " << mode.c_str() << endl;
	drawString(windowSize.x - 170, windowSize.y - 60, s.str());
	s.str("");
	s << "fov (z/Z)              : " << fov << endl;
	drawString(windowSize.x - 170, windowSize.y - 45, s.str());
	s.str("");
	s << "light distance (l/L) : " << setprecision(2) << lightDistance << endl;
	drawString(windowSize.x - 170, windowSize.y - 30, s.str());
	s.str("");
	s << "shininess (s/S)      : " << setprecision(2) << material.shininess << endl;
	drawString(windowSize.x - 170, windowSize.y - 15, s.str());
...
#ifdef WIN32 
	msecTime = timeGetTime();
#endif

	shadingDemo->draw();

#ifdef WIN32 
	msecTime = timeGetTime() - msecTime;
#endif
...
void Context::keyPressed(unsigned char key, int, int) {
	...
	switch(key) {
		...
		case 's':
		if(material.shininess > 0.005) material.shininess *= 0.99;
		break;

		case 'S':
		material.shininess *= 1.01;
		break;
		...
		case 'm':
		shadingDemo->setRenderMode(
			(shadingDemo->getRenderMode() == VERTEX_ARRAY) ?
...
int main(int argc, char** argv) {
	...
	// load mesh
	shadingDemo->loadMesh(argc > 1 ? argv[1] : "meshes/eight.off");
